              ...
              <TableRow className="hover:bg-transparent border-none">
                <TableCell colSpan={colSpan} className="h-72 text-center">
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-sm font-medium text-zinc-400">{loadingState ?? "Fetching records..."}</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : data.length === 0 ? (
              <TableRow className="hover:bg-transparent border-none">
                <TableCell colSpan={colSpan} className="h-72 text-center">
                  <div className="flex flex-col items-center justify-center gap-2 text-zinc-500">
                    <Inbox size={48} strokeWidth={1} className="mb-2 opacity-10" />
                    <p className="text-sm font-semibold text-zinc-300">{emptyState ?? "No records found"}</p>
                    <p className="text-xs text-zinc-500">Adjust filters or check back later.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
          ...
          <div className="flex gap-1.5">
            {[...Array(totalPages)].map((_, i) => {
              if (totalPages > 5 && Math.abs(i + 1 - page) > 1 && i !== 0 && i !== totalPages - 1) {
                if (Math.abs(i + 1 - page) === 2) return <span key={i} className="text-zinc-700 self-end px-1">...</span>;
                return null;
              }
              return (
                <button
