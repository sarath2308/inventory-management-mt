...
export const ITEM_API = {
    ...
    UPATE:(itemId: string,payload:Partial<CreateItemType>)=> api.patch(`/items/${itemId}`,payload).then((res)=>res.data),
    REMOVE:(itemId: string) => api.delete(`/items/${itemId}`).then((res)=>res.data),
    GET_ITEMS:(search: string, page: number) => api.get(`/items?search=${search}&page=${page}`).then((res)=>res.data),
}